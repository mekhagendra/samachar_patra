...
class SP_Technology_Widget extends WP_Widget {
    
    public function __construct() {
        parent::__construct(
            'sp_technology_widget',
            __('SP: Technology', 'samacharpatra'),
            array(
                'description' => __('Display technology news section', 'samacharpatra'),
                'classname' => 'sp-technology-widget'
            )
        );
    }

    public function widget($args, $instance) {
        $title = !empty($instance['title']) ? $instance['title'] : 'प्रविधि';
        $posts_count = !empty($instance['posts_count']) ? absint($instance['posts_count']) : 4;
        $category = !empty($instance['category']) ? sanitize_text_field($instance['category']) : 'technology';

        // Set query vars for template
        set_query_var('technology_title', $title);
        set_query_var('technology_posts_count', $posts_count);
        set_query_var('technology_category', $category);

        echo $args['before_widget'];
        
        // Load template
        get_template_part('templates/components/technology');
    ...
    public function form($instance) {
        $title = !empty($instance['title']) ? $instance['title'] : 'प्रविधि';
        $posts_count = !empty($instance['posts_count']) ? absint($instance['posts_count']) : 4;
        $category = !empty($instance['category']) ? $instance['category'] : 'technology';
        ?>
        <p>
    ...
    public function update($new_instance, $old_instance) {
        ...
        $instance['category'] = !empty($new_instance['category']) ? sanitize_text_field($new_instance['category']) : 'technology';
