...
class SP_Trending_Widget extends WP_Widget {
    
    public function __construct() {
        parent::__construct(
            'sp_trending_widget',
            __('SP: Trending News', 'samacharpatra'),
            array(
                'description' => __('Display trending news section', 'samacharpatra'),
                'classname' => 'sp-trending-widget'
            )
        );
    }

    public function widget($args, $instance) {
        $title = !empty($instance['title']) ? $instance['title'] : 'ट्रेन्डिङ';
        ...
        set_query_var('trending_title', $title);
        set_query_var('trending_posts_count', $posts_count);
        set_query_var('trending_category', $category);

        echo $args['before_widget'];
        
        // Load template
        get_template_part('templates/components/trendings');
