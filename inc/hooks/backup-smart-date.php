 ...
 */
function smart_date_get_user_location() {
 ...
 */
function smart_date_get_bs_data() {
    static $bs_data = null;
 ...
 */
function smart_date_gregorian_to_bs($timestamp) {
    ...
    $bs_data = smart_date_get_bs_data();
 ...
 * @return string
 */
function smart_date_to_nepali_digits($number) {
    $nepali_digits = array(
        '0' => '०', '1' => '१', '2' => '२', '3' => '३', '4' => '४',
        '5' => '५', '6' => '६', '7' => '७', '8' => '८', '9' => '९'
    );
...
function smart_date_short($timestamp = null) {
    ...
    $location = smart_date_get_user_location();
    
    if ($location === 'nepal') {
        $bs_date = smart_date_gregorian_to_bs($timestamp);
        $day_nepali = smart_date_to_nepali_digits($bs_date['day']);
 ...
 */
function smart_date_short_time($timestamp = null) {
    if ($timestamp === null) {
        $timestamp = current_time('timestamp');
    }
    ...
    if ($location === 'nepal') {
        ...
        $hour_nepali = smart_date_to_nepali_digits($hour);
        $minute_nepali = smart_date_to_nepali_digits($minute);
...
function smart_date_full($timestamp = null) {
    ...
    if ($location === 'nepal') {
        $bs_date = smart_date_gregorian_to_bs($timestamp);
        $year_nepali = smart_date_to_nepali_digits($bs_date['year']);
        $day_nepali = smart_date_to_nepali_digits($bs_date['day']);
 ...
 */
function smart_date_full_time($timestamp = null) {
    if ($timestamp === null) {
        $timestamp = current_time('timestamp');
    }
...
function smart_date_relative($timestamp = null) {
    ...
    if ($location === 'nepal') {
        if ($diff < 60) {
            ...
            $minutes_nepali = smart_date_to_nepali_digits($minutes);
            return $minutes_nepali . ' मिनेट अघि';
        } elseif ($diff < 86400) {
            $hours = floor($diff / 3600);
            $hours_nepali = smart_date_to_nepali_digits($hours);
            return $hours_nepali . ' घण्टा अघि';
        } else {
            $days = floor($diff / 86400);
            $remaining_hours = floor(($diff % 86400) / 3600);
            
            $days_nepali = smart_date_to_nepali_digits($days);
            
            if ($remaining_hours > 0 && $days < 7) {
                $hours_nepali = smart_date_to_nepali_digits($remaining_hours);
 ...
 */
function the_smart_date_short($timestamp = null) {
    echo esc_html(smart_date_short($timestamp));
}

function the_smart_date_short_time($timestamp = null) {
    echo esc_html(smart_date_short_time($timestamp));
}

function the_smart_date_full($timestamp = null) {
    echo esc_html(smart_date_full($timestamp));
}

function the_smart_date_full_time($timestamp = null) {
    echo esc_html(smart_date_full_time($timestamp));
}

function the_smart_date_relative($timestamp = null) {
    echo esc_html(smart_date_relative($timestamp));
}
